predictiveText <- function(characterString, wordCloud=FALSE) {
  ...
  if(l >= 3) {
    ...
    if(length(possible[,1]) > 0) {
      ...
      output <- data.frame(Words = possible$gram, Freq = possible$Record)
    }
    ...
    if(length(possible[,1]) == 0) {
      ...
      # if the string can't be found in the three-gram, then dive into the two-gram
        ...
        # if the string still cannot be found, generate a random fun word
          ...
          output <- data.frame(Words = randomWords, Freq = floor(runif(n, 1, 101)))
        }
      }
    }
  }
  ...
  if(wordCloud == FALSE) {
    ...
    output <- as.character(output[output$Freq == max(output$Freq),'Words'])
  } 
